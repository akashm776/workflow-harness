...
from compiler.static_validation import validate_invalid_edge_endpoints
...
INVALID_FIXTURE = ROOT / "fixtures" / "invalid" / "invalid-edge-endpoint"


class InvalidEdgeEndpointValidationTests(unittest.TestCase):
    def test_valid_fixtures_pass_invalid_edge_endpoint_validation(self) -> None:
        for fixture_name in VALID_FIXTURES:
            workflow_spec_path = (
                ROOT / "fixtures" / "valid" / fixture_name / "input" / "WorkflowSpec.json"
            )
            result = validate_invalid_edge_endpoints(workflow_spec_path)

            with self.subTest(fixture=fixture_name):
                self.assertTrue(result["ok"])
                self.assertIsNone(result["diagnostic"])

    def test_invalid_fixture_fails_invalid_edge_endpoint_validation(self) -> None:
        expected = json.loads(
            (INVALID_FIXTURE / "expected" / "error.json").read_text(encoding="utf-8")
        )
        result = validate_invalid_edge_endpoints(INVALID_FIXTURE / "input" / "WorkflowSpec.json")
